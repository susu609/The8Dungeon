...
import static net.ss.sudungeon.event.HurtEventHandler.latestDamageAmount;
...
public class ParticleNumeric extends Particle {
    ...
    public ParticleNumeric (ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        ...
        this.outlineColor = (this.customColor & 0x00FFFFFF) | 0x80000000;
        this.lifetime = 35;
        this.fadeout = 1.0F;
        this.setSize(2f,2f);
    ...
    public void render (@NotNull VertexConsumer buffer, Camera camera, float partialTicks) {
        ...
        String numberString = (this.number % 1 == 0)
                ? String.valueOf((int) this.number)
                : String.format("%.1f", this.number);
        ...
        int colorOutlineWithAlpha = this.outlineColor & 0x00FFFFFF | (int) (currentAlpha * 255) << 24;
        ...
        // Vẽ số sát thương với viền 8x
        MultiBufferSource.BufferSource multiBufferSource = Minecraft.getInstance().renderBuffers().bufferSource();
        Matrix4f matrix = matrixStack.last().pose();

        // Chuyển đổi numberString thành FormattedText
        FormattedText formattedText = Component.literal(numberString);

        // Sử dụng drawInBatch8xOutline để vẽ số với viền
        if ((outlineColor & 0x00FFFFFF) != (customColor & 0x00FFFFFF)) {
            // Chỉ vẽ viền nếu màu khác nhau
            fontRenderer.drawInBatch8xOutline(
                    fontRenderer.split(formattedText, Integer.MAX_VALUE).get(0),
                    -fontRenderer.width(numberString) / 2.0F,
                    0,
                    colorWithAlpha,
                    colorOutlineWithAlpha,
                    matrix,
                    multiBufferSource,
                    15728880
            );
        } else {
            // Không có viền, vẽ bình thường
